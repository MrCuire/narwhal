...
function is_mobile_phone ($mobile_phone)
{
    ...
    $chars = "/^13[0-9]{1}[0-9]{8}$|15[0-9]{1}[0-9]{8}$|14[0-9]{1}[0-9]{8}$|16[0-9]{1}[0-9]{8}$|17[0-9]{1}[0-9]{8}$|18[0-9]{1}[0-9]{8}$|19[0-9]{1}[0-9]{8}$/";
...
function is_idcard($idcard){
    $chars = '/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/';
    if(preg_match($chars,$idcard)){
        return true;
    }
    return false;
}

/*验证时间格式必须为 2010.10.10 格式*/
function is_data($data){
    $chars = '/^[1-9]\d{3}\.([1-9]|0[1-9]|1[0-2])\.([1-9]|0[1-9]|[1-2][0-9]|3[0-1])$/';
...
//         $arr['ww']                   = !empty($arr['ww']) ? $arr['ww'] : '';
        ...
        /* fulltext_search_add_START_www.charse.net */
	$arr['fulltext_search']            = isset($arr['fulltext_search']) ? $arr['fulltext_search'] : 0;  
        /* fulltext_search_add_END_www.charse.net */
 ...
 */
// 代码修改_start_derek20150129admin_goods  www.charse.net
function get_brand_list($t = false)
// 代码修改_end_derek20150129admin_goods  www.charse.net
{
...
{
/* 代码增加_虚拟团购_START  www.charse.net */
...
function clear_tpl_files($is_cache = true, $ext = '')
{
    ...
    foreach ($dirs AS $dir)
    {
        ...
        {
            if ($file == '.' || $file == '..' || $file == 'index.htm' || $file == 'index.dwt')
            {
                continue;
            }
            if (is_file($dir . $file))
...
/**
 *  清除手机指定后缀的模板缓存或编译文件
 *  wei2 增加 start by www.charse.net
 * @access  public
 * @param  bool       $is_cache  是否清除缓存还是清出编译文件
 * @param  string     $ext       需要删除的文件名，不包含后缀
 *
...
function smarty_insert_scripts_my($args)
{
    static $scripts = array();
    ...
    foreach ($arr AS $val)
    {
        if (in_array($val, $scripts) == false)
        {
            ...
            if ($val{0} == '.')
            {
                $str .= '<script type="text/javascript" src="'.SITE_URL . substr($val,1) . '"></script>';
            }
            else
            {
                $str .= '<script type="text/javascript" src="'.SITE_URL .ADMIN_PATH.'/js/' . $val . '"></script>';
            }
        }
    }
 ...
 * @param   int     $change_role_type    0：系统；1：管理员ID；2：代理商ID
 * @param   int     $change_type    0:普通用户；1：征信代理；3：org_user;4:车代理；5:车平台；
 * @return  void
 */
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0,
                            $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER,$user_type=0,
                            $change_role_type=0,$change_role_id=0,$operate_type=0)
{
  $orderid='';
    if($user_type==0){
        $user_table='users';
        ...
        $user_table='supplier_admin_user';
        $log_table="supplier2_account_log";     
        $sql = 'SELECT user_money FROM ' . $GLOBALS['ecs']->table('supplier_admin_user') . " WHERE supplier_id = '$user_id'";      
        $first_money = $GLOBALS['db']->GetOne($sql);            
    }elseif($user_type==3){
        $orderid=uniqid().time();
        $sql = 'SELECT user_money FROM ' . $GLOBALS['ecs']->table('org_users') . " WHERE orgId = '$user_id'";      
        $first_money = $GLOBALS['db']->GetOne($sql);    
        $user_table='org_users';
        $log_table="org_account_log";
    }elseif($user_type==4){
        $user_table='car_supplier_admin_user';
        $log_table="car_supplier_account_log";
    }elseif($user_type==5){
        $user_table='car_platform';
        $log_table="car_platform_account_log";
    }

    /* 插入帐户变动记录 */
    $account_log = array(
        'first_money'  => $first_money,
        'last_money'  => $first_money+$user_money,
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table($log_table), $account_log, 'INSERT');
  
    /* 更新用户信息 */
    $sql = "UPDATE " . $GLOBALS['ecs']->table($user_table) .
    ...
    if($user_type==0){
        ...
        $sql.=" WHERE supplier_id = '$user_id' LIMIT 1";
    }elseif($user_type==3){
        $sql.=" where `orgId`='$user_id' LIMIT 1";
    }elseif($user_type==4){
        $sql.=" where `supplier_id`='$user_id' LIMIT 1";
    }elseif($user_type==5){
        $sql.=" where `o_id`='$user_id' LIMIT 1";
    }
    $GLOBALS['db']->query($sql);
    return "success";
}
 ...
 * @param   string  $change_desc    变动说明
 * @param   int     $change_type    变动类型：参见常量文件
 * @param   int     $user_type    0:普通用户；1：征信代理；3：org_user;4:车代理；5:车平台；
 * @return  void
 */
function log_tcbi_account_change($user_id, $user_tcbi = 0, $change_desc = '', $change_type = ACT_OTHER,$user_type=0,$operate_user_id=0)
{
    if($user_type==1){
        $user_table='supplier_admin_user';
        $log_table="supplier_tcbi_account_log";
        $sql = 'SELECT `tcbi` FROM ' . $GLOBALS['ecs']->table('supplier_admin_user') . " WHERE supplier_id = '$user_id'";
        $first_tcbi = $GLOBALS['db']->GetOne($sql);
    }

    /* 插入帐户变动记录 */
    $account_log = array(
        'first_tcbi'  => $first_tcbi,
        'last_tcbi'  => $first_tcbi+$user_tcbi,
        'user_id'       => $user_id,
        'user_tcbi'    => $user_tcbi,
        'change_time'   => gmtime(),
        'change_desc'   => $change_desc,
        'change_type'   => $change_type,
        'operate_user_id'=>empty($operate_user_id)?$user_id:$operate_user_id,
    );
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table($log_table), $account_log, 'INSERT');

    /* 更新用户信息 */
    $sql = "UPDATE " . $GLOBALS['ecs']->table($user_table) .
        " SET tcbi = tcbi + ('$user_tcbi')" ;
    if($user_type==1){
        $sql.=" WHERE supplier_id = '$user_id' LIMIT 1";
    }
    $GLOBALS['db']->query($sql);
...
function get_package_goods($package_id, $package_attr_id='')
{
	...
	if ($package_attr_id)
	{
	 ...
	 // 下面SQL语句增加两个字段，注意逗号 , g.goods_thumb, g.shop_price   By  www.charse.net
...
function Recordkeyword($word_www_68ecshop_com, $items = 0, $searchengine = 'ecshop')
{
	...
	if($r)
	{
		$items = intval($items) ;   //www.charse.net
...
function make_html()
{
	...
	$makeurl = !empty($makeurl) ? $makeurl : "index.dwt"; //特殊值
...
{
    //apiKey
    $apiKey = "068142e17bd9482f96646e5a54613dcb";
    //应用appId值
    $appId = "RA278744626o";
    //手机号
    $mobile = $mobile;
 ...
 * @return  array
 */
function car_sendSMS ($mobile, $content, $time = '', $mid = '')
{
    //apiKey
    $apiKey = "068142e17bd9482f96646e5a54613dcb";
    //应用appId值
 ...
 * 签名：时间征信
 * @access  public
 * @params  string $PhoneNumbers 手机号码  15817112071
 * @params  string $TemplateCode 短信模板编号  SMS_105155081
 * @params  string $TemplateCode 短信模板变量   Array("code"=>"1234")
 * @return  boolean
 */
function sendSMSALI ($PhoneNumbers,$TemplateParam,$TemplateCode="SMS_137422328",$signName="时间科技")
{
    include_once(LIB_PATH.'/modules/sms/ali_sms/Sms.php');
    $sms = new Sms(
        "LTAIBM3tDZN07F24",
        "hDz8nIgak5TE245Q4md5adeTginHrR"
    );
 ...
 * @return  
 */
function registerByInviteCode($uId, $inviteCode)
{
    $sql   = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('user_invite_code')." where `invite_code`='$inviteCode' and `use_time`=0";
    $count = $GLOBALS['db']->getOne($sql);
    if($count>0){
        $sql   = "SELECT `car_supplier_id` FROM " .$GLOBALS['ecs']->table('users')." where `user_id`='$uId'";
        $car_supplier_id = $GLOBALS['db']->getOne($sql);
        if($car_supplier_id>0){
            //此用户已绑定代理商，不能再次绑定；
            return null;
        }
        //更新邀请码使用状态
        $sql = "UPDATE ".$GLOBALS['ecs']->table('user_invite_code'). " SET `use_time`=".time().",`c_id`=".$uId." where `invite_code`='$inviteCode'";
        $GLOBALS['db']->query($sql);
        $sql   = "SELECT * FROM " .$GLOBALS['ecs']->table('user_invite_code')." where `invite_code`='$inviteCode'";
        $inviteCodeInfo = $GLOBALS['db']->getRow($sql);
        if($inviteCodeInfo['type']=='1'){
            //需要查询oemid
            $sql   = "SELECT * FROM " .$GLOBALS['ecs']->table('car_supplier_admin_user')." where `supplier_id`='$inviteCodeInfo[f_id]'";
            $supplierInfo = $GLOBALS['db']->getRow($sql);
            return array("car_supplier_id"=>$inviteCodeInfo['f_id'],"car_o_id"=>$supplierInfo['o_id']);
        }else{
            return array("car_supplier_id"=>"0","car_o_id"=>$inviteCodeInfo['f_id']);
        }
 ...
 * */
function app_alipay(){
    include_once(ROOT_PATH.'includes/modules/payment/appalipay/alipay.php');
    return new alipayAppPay();
}
/*
 * App 微信
 * */
function app_wxpay(){
    include_once(ROOT_PATH.'includes/modules/payment/appwxpay/wxpay.php');
    return new wxAppPay();
}
 ...
 * */
function nps_pay(){
    include_once(ROOT_PATH.'includes/modules/payment/npspay/npspay.php');
    return new npsPay();
}
/*
 * 生成第三方推送别名
...
function getAlias($uid){
    $string='etykwropmqi';
    return substr($string,0,(11-strlen($uid))).$uid;
}
/**
 * 发送站内信和邮箱函数
 *
 * @param   string      $Sender     发送人
 * @param   array       $sendtype   发送方式  包含0：站内信；包含1：邮件；
 * @param   array       $RecID      接收消息代理商ID序列
 * @param   string      $usetem     2:不使用模板；1：使用模板 
 * @param   string      $theme      发送主题
 * @param   string      $content    发送内容
 * @param   array       $messtemlist   模板ID序列
 * @param   string      $from    1:平台发送给代理；2：代理回复给平台
 *
 * @return  boolean
 */
function sendmess_email($Sender,$RecID,$content,$theme,$from='1',$sendtype=array('0'=>'0'),$usetem='2',$messtemlist='')
{     
    $date = time();
    if($usetem==2){
        if (empty($theme))
        {
            sys_msg("主题不能为空", 1, array(), false);
        }
        else
        {
            $theme = trim($theme);
    ...
    if($usetem==1){
        if (empty($messtemlist))
        {
            sys_msg("请选择发送模板", 1, array(), false);
        }
    }
    ...
    foreach ($suppliers_list as $key => $value) {
        $newkey=$suppliers_list[$key]['supplier_id'];
        $allemaillist[$newkey]=$suppliers_list[$key]['email'];
    }
    ...
    if($usetem==2){
        ...
        $sql = "insert into ".$GLOBALS['ecs']->table('supplier_messcontent')." (`theme`,`content`,`Date`) values('$theme','$content',$date)";

        $result = $GLOBALS['db']->query($sql);

        $lastid = "SELECT `c_id` from ".$GLOBALS['ecs']->table('supplier_messcontent')." order by c_id desc limit 0,1";
        $lastid = $GLOBALS['db']->getOne($lastid);
      
        foreach ($RecID as $value) {
           
            $aql = "insert into ".$GLOBALS['ecs']->table('supplier_messlist')." (`Sender`,`RecID`,`Statue`,`contentID`,`sendletter`,`sendemail`,`sendby`) values('$Sender','$value',$Statue,$lastid,$sendletter,$sendemail,$from)";
            $GLOBALS['db']->query($aql);

        }
    }
    if($usetem==1){
        foreach ($messtemlist as $contentid) {
            foreach ($RecID as $value) {
                $sql = "insert into ".$GLOBALS['ecs']->table('supplier_messlist')." (`Sender`,`RecID`,`Statue`,`contentID`,`sendletter`,`sendemail`) values('$Sender','$value',$Statue,$contentid,$sendletter,$sendemail)";
                $GLOBALS['db']->query($sql);
            }
        }
    }
    if($sendemail=='1'){//发送邮件
        foreach ($RecID as $value) {
            echo '发送邮件给'.$allemaillist[$value].'</br>';
            $content=str_replace('\"', '', $content);
            send_mail("",$allemaillist[$value],$theme,$content,1);
        }
    }
    return true;
}
...
function get_suppliers_list(){
    global $ecs,$db;
    $sql  = 'SELECT * FROM ' .$GLOBALS['ecs']->table('supplier_admin_user');
    $list = $GLOBALS['db']->getAll($sql);
    return $list;
}
function getCoordinates($ip){
    $content = file_get_contents("http://api.map.baidu.com/location/ip?ak=YWNt8VcHK7Goj1yljLlMVHnWl6ZWS26t&ip={$ip}&coor=bd09ll");    
    $json = json_decode($content);
    if($json->{'status'} =='0'){
        return $json->{'content'}->{'point'}->{'y'}.','.$json->{'content'}->{'point'}->{'x'};
    }else{
        return '';        
    }    
}
