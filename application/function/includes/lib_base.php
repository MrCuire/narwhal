 ...
 * 消息推送
 * */
function sendMessage($uids,$message,$utype=1){
    // 建立socket连接到内部推送端口
    if(defined('INNER_MSG_PORT')){
       // error_reporting(E_ALL);
        $client = stream_socket_client('tcp://'.INNER_MSG_PORT, $errno, $errmsg, 1, STREAM_CLIENT_CONNECT);
        $message=base64_encode($message);
        $data = array('uid'=>$uids, 'message'=>$message,'utype'=>$utype);
...
function SafeFilter (&$arr)
{
    ...
    $ra=Array('/([\x00-\x08,\x0b-\x0c,\x0e-\x19])/','/script/','/javascript/','/vbscript/','/expression/','/applet/','/meta/','/xml/','/blink/','/style/','/embed/','/object/','/frame/','/layer/','/title/','/bgsound/','/base/','/onload/','/onunload/','/onchange/','/onsubmit/','/onreset/','/onselect/','/onblur/','/onfocus/','/onabort/','/onkeydown/','/onkeypress/','/onkeyup/','/onclick/','/ondblclick/','/onmousedown/','/onmousemove/','/onmouseout/','/onmouseover/','/onmouseup/','/onunload/');
 ...
 * $expiry 缓存时间
 * */
function save_cache($name,$value,$expiry=30){
    $skey=trim($name);
    $time=time()+$expiry;
    $sql="insert into ".$GLOBALS['ecs']->table('cache_data')."( `skey`,`expiry`,`data`) values ('$skey',$time,'$value') ON DUPLICATE KEY UPDATE `expiry`=$time , `data`='$value'";
...
function get_cache($name){
    $skey=trim($name);
    $time=time();
    $sql="select `data` from".$GLOBALS['ecs']->table('cache_data')." where `skey`='$skey' and `expiry`>$time";
    return $GLOBALS['db']->getOne($sql);

}
function clear_db_cache(){
    truncate_table('cache_data');
}
function imageComPress($imgsrc,$imgdst,$ratio=0.8){
