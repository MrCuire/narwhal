...
function _register ($username, $password, $email_or_mobile, $other = array(), $register_type = 'email')
{
	...
	else
	{
		...
		//手机绑定 车代理 和 oem
		if(!empty($mobile)){
			$supplier_id=$GLOBALS['db']->getOne("select `supplier_id` from ".$GLOBALS['ecs']->table('car_bindingphone')." where `mobile_phone`='$mobile'");
			if(!empty($supplier_id)){
				$o_id=$GLOBALS['db']->getOne("select `o_id` from ".$GLOBALS['ecs']->table('car_supplier_admin_user')." where `supplier_id`=$supplier_id");
				if(!empty($o_id)){
					$update_data['car_supplier_id']=$supplier_id;
					$update_data['car_o_id']=$o_id;
				}
			}
		}
